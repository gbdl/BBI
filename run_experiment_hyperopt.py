#Interface between main.py and hyperopt.
...
def run_experiment(sysargv = ["main.py", "PDE_PoissonD"]):
    ...
    args_main = parser.parse_args(sysargv[1:2])
    experiment = eval(args_main.experiment + "()")
    parser = argparse.ArgumentParser(
        parents=[experiment.parser()], prog=sysargv[0] + " " + sysargv[1]
    )
    ...
    parser.add_argument("--deltaEn", default=0.0, type=float, help="Extra energy for the BBI")
    ...
    parser.add_argument(
        ...
        default=0.0,
        type=float,
        help="L2-regularization. For linear optimizers, it is identical to weight decay.",
    )
    parser.add_argument("--rho", default=0.9, type=float, help="Momentum")

    parser.add_argument("--v0", default=0, type=float, help="Delta V for the DBI optimizer.")
    ...
    # set the seed and use deterministic algorithms
    ...
    return experiment.stats["loss train"][-1] 
